...
function parseZodSchemaFromSource(source, schemaName) {
	...
	// Find the schema definition - look for z.object({ ... })
	const schemaStartRegex = new RegExp(
		`(?:const|let|var)\\s+${schemaName}\\s*=\\s*z\\.object\\(\\{`,
		"m"
	);
...
function parseParamContent(name, content) {
	...
	// Match .describe( followed by quote, then capture everything until closing quote and )
	const descMatch = content.match(/\.describe\s*\(\s*["'`]([\s\S]*?)["'`]\s*[,)]/);
	...
	if (baseType === "unknown" || !["string", "number", "boolean", "array", "object", "enum", "union", "literal"].includes(baseType)) {
...
function parseToolFromSource(source, filePath) {
	...
	for (const nameMatch of nameMatches) {
		...
		// Find the description near this name (within ~500 chars)
...
function extractZodV4BaseType(prop) {
	...
	switch (wrapperType) {
		...
		case "enum": return { type: "string", enum: def.entries ? Object.keys(def.entries) : [] };
		case "array": return { type: "array" };
		case "object": return { type: "object" };
		default: return { type: wrapperType || "unknown" };
	}
}
...
function zodV4ToJsonSchema(schema) {
	...
	for (const [key, prop] of Object.entries(shape)) {
		...
		properties[key] = { type: typeInfo.type, description: prop.description || "" };
		if (typeInfo.enum) properties[key].enum = typeInfo.enum;
		if (typeInfo.default !== undefined) properties[key].default = typeInfo.default;
		if (!typeInfo.optional && typeInfo.default === undefined) required.push(key);
	}
...
async function zodToJsonSchema(schema) {
	...
	if (schema && typeof schema.safeParse === "function") {
		try {
			const { zodToJsonSchema: zodV3ToJsonSchema } = await import("zod-to-json-schema");
	...
	for (const file of files) {
		...
		try {
			...
			if (parsedTools.length > 0) {
				console.log(`Parsed ${parsedTools.length} tool(s) from ${file} via source`);
			}
		} catch (err) {
...
function renderSchema(jsonSchema) {
	...
	const publicProperties = Object.entries(properties).filter(([key]) => !key.startsWith("_"));
	if (publicProperties.length === 0) return "_No parameters_";

	const hasDefaults = publicProperties.some(([, prop]) => prop.default !== undefined);
